...
do
  p=$(basename $platform)
  ASSET_FILENAME="monitoring-plugins-${p}_${ASSET_VERSION}.tar.gz"
  ...
  docker build --no-cache --rm --build-arg "PLUGINS=$PLUGINS" --build-arg "SENSU_GO_ASSET_VERSION=$ASSET_VERSION" -t monitoring-plugins-${p}:$ASSET_VERSION -f builds/enabled/${p}/Dockerfile .
  docker cp -L $(docker create --rm monitoring-plugins-${p}:$ASSET_VERSION true):/$ASSET_FILENAME ./assets/
...
[ ! -f "./assets/${filename}" ] && echo "Error: bonsai asset_filename ${f} missing from assets directory!" && exit 1
done



cd assets/
CHECKSUMS_FILENAME="monitoring-plugins_${ASSET_VERSION}_sha512-checksums.txt"

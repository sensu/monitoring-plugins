...
sudo docker build --build-arg "PLUGINS=$PLUGINS" --build-arg "SENSU_GO_ASSET_VERSION=$SENSU_GO_ASSET_VERSION" -t monitoring-plugins-centos:$SENSU_GO_ASSET_VERSION -f Dockerfile.centos .
sudo docker run -v "$PWD/assets:/assets" monitoring-plugins-centos:$SENSU_GO_ASSET_VERSION cp /$SENSU_GO_ASSET_FILENAME /assets/centos/$SENSU_GO_ASSET_FILENAME
